...
#
#                 # 将特征转换为numpy数组 / convert face descriptor to numpy array
#                 face_descriptor_list = []
#                 for num in range(0, 128):
#                     face_descriptor_list.append(face_descriptor[num])
#                 self.current_frame_face_feature_list.append(face_descriptor_list)
#
#                 # 获得面部标志 / get face landmark
#                 for i in range(68):
#                     cv2.circle(img_rd, (shape.part(i).x, shape.part(i).y), 5, (0, 255, 0), 2)
#
#                 # 计算质心坐标 / calculate the centroid of face
#                 x = d.left()
#                 y = d.top()
#                 w = d.right() - x
#                 h = d.bottom() - y
#                 cx = x + w // 2
#                 cy = y + h // 2
#                 centroid_current_frame = (cx, cy)
#
#                 # 记录质心坐标和人脸名字 / record centroid position and face name
#                 self.current_frame_face_centroid_list.append(centroid_current_frame)
#                 self.current_frame_face_name_list.append("unknown")
#
#                 # 识别过程 / recognize process
#                 for i in range(len(self.current_frame_face_feature_list)):
#                     for person_num in range(len(self.face_features_known_list)):
#                         e_distance_tmp = self.return_euclidean_distance(
#                             self.current_frame_face_feature_list[i], self.face_features_known_list[person_num])
...
#
#                     # 清除用于识别的列表 / clear list for recognize
#                     del self.current_frame_face_X_e_distance_list[:]
...
#     #                    b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')
#
#     # # 在视频流上运行人脸识别 / Run face recognition on video stream
#     # def run(self, stream):
#     #     self.process(stream)
#
...
class Face_Recognizer:
    ...
    def run(self):
        cap = cv2.VideoCapture(0)  # Get video stream from video file
        #cap = cv2.VideoCapture("rtmp://101.200.135.114:9090/live/live0")              # Get video stream from camera
        self.process(cap)
