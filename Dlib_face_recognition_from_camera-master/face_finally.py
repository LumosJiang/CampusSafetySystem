...
class Face_Recognizer:
    def __init__(self):
        ...
        self.unknown_logged_times = set()  # 用集合来记录已经记录过的时间戳
    ...
    def log_forbidden_event(self, event_time, location, event_type):
        try:
            ...
            if connection.is_connected():
                connection.close()
    def log_unknown_event(self, event_time, location, event_type):
        try:
            # 获取当前目录下已有的视频文件数量
            video_dir = r'D:\nodejsProjects\project11\Frontend\public\pictures\unknown'
            current_videos = [filename for filename in os.listdir(video_dir) if
                              filename.startswith('output') and filename.endswith('.jpg')]
            video_count = len(current_videos)
            ...
            # 新视频文件路径和名称
            video_name = f'output_{video_count}.jpg'
            video_path = os.path.join("../../public/pictures/unknown/", video_name)
    ...
    def save_unknown_face_image(self, frame):
        # 确保目录存在，如果不存在则创建
        unknown_faces_dir = 'D:/nodejsProjects/project11/Frontend/public/pictures/unknown'
        if not os.path.exists(unknown_faces_dir):
            os.makedirs(unknown_faces_dir)

        current_videos = [filename for filename in os.listdir(unknown_faces_dir) if
                          filename.startswith('output') and filename.endswith('.jpg')]
        video_count = len(current_videos)
    ...
    def get_next_video_filename(self):
        ...
        try:
            ...
            cursor.execute(insert_query, (next_video_id, video_path, video_datetime, video_name, "人脸识别"))
            conn.commit()
            cursor.close()
            conn.close()
            ...
            while stream.isOpened():
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    if "unknown" in self.current_frame_face_name_list:
                        ...
                        # 只有当当前时间戳不在已记录的集合中时，才记录事件
                        if current_time not in self.unknown_logged_times:
                            self.reclassify_interval_cnt += 1
                            self.log_unknown_event(event_time=current_time,
                                                     location="教室",
                                                     event_type="未知人员")

                            # 将当前时间戳添加到已记录集合中，以确保同一秒内只记录一次事件
                            self.unknown_logged_times.add(current_time)
                            # 保存未知人脸图片
                            self.save_unknown_face_image(img_rd)
